                ...
                if let Some(user_agent) = headers.get("User-Agent") {
                    match user_agent.to_str() {
                        Ok(agent) if agent.to_lowercase().starts_with("sonarr") => {
                            ...
                            warp::reply::with_status("Processing Sonarr data", warp::http::StatusCode::OK)
                        }
                        Ok(agent) if agent.to_lowercase().starts_with("unraid") => {
                            println!("Processing Unraid data");
                            warp::reply::with_status("Processing Unraid data", warp::http::StatusCode::OK)
                        }
                        _ => {
                            // Unsupported content type
                            println!("Received unsupported User-Agent");
                            warp::reply::with_status("Received unsupported User-Agent", warp::http::StatusCode::BAD_REQUEST)
                        }
                    }
                } else {
                    // If there is no User-Agent header, return a 400 Bad Request.
                    warp::reply::with_status("No User-Agent header", warp::http::StatusCode::BAD_REQUEST)
                }
            }
        });
